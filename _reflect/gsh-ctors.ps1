...
Write-Host 'ctors:'
$gsh.GetConstructors($flags) | ForEach-Object { $_.ToString() }
Write-Host 'fields:'
$gsh.GetFields($flags) | ForEach-Object { $_.ToString() }
$pam = $asm.GetType('CustomAvatar.Player.PlayerAvatarManager')
$pam.GetFields($flags) | Where-Object { $_.Name -match 'rig|Rig|settings|Settings|tracking' } | ForEach-Object { $_.ToString() }
